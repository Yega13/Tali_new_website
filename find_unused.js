...
function getAllPhotos(dir) {
    ...
    return fs.readdirSync(dir).filter(file => {
        ...
        return stat.isFile() && !file.startsWith('.'); // Ignore hidden files/directories
    });
}
...
srcFiles.forEach(file => {
    ...
    // Matches: /photos/filename.ext or just filename.ext inside quotes
    // We'll simplisticly check if the filename exists in the content

    allPhotos.forEach(photo => {
        // Check for exact filename match to avoid partials (e.g. "pic.jpg" matching "pic.jpg.bak")
        // But code usually references "/photos/pic.jpg" or just "pic.jpg"
